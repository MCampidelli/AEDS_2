...
//Questão 01 ----- Ordenação PARCIAL por Seleção em Java
...
class Restaurante {
    ...
    public static Restaurante parseRestaurante(String linha) {
        ...
        restaurante.capacidade = Integer.parseInt(campos[3]);
        restaurante.avaliacao = Double.parseDouble(campos[4]);

        restaurante.tiposCozinha = campos[5].split(";");
        restaurante.quantidadeTipos = restaurante.tiposCozinha.length;

        restaurante.faixaPreco = campos[6].length();

        String[] horarios = campos[7].split("-");
        restaurante.horarioAbertura = Hora.parseHora(horarios[0]);
        restaurante.horarioFechamento = Hora.parseHora(horarios[1]);

        restaurante.dataAbertura = Data.parseData(campos[8]);

        restaurante.aberto = campos[9].equals("true");
...
public class Selecao {
    ...
    public static void gerarLog(long tempo) throws Exception {

        PrintWriter pw = new PrintWriter(new FileWriter("810688_selecao_parcial.txt"));
    ...
    public static void main(String[] args) throws Exception {
        ...
        while (id != -1) {
            ...
            array[n++] = colecao.buscarPorId(id);
