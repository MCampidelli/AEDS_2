...
void swap (Restaurante** array, int a, int b) {
     Restaurante* temp = array[a];
     array[a] = array[b];
     array[b] = temp;
}

void quicksortRec (Restaurante** array, int esq, int dir, long* comp, long* mov) {
     int i = esq, j = dir;
     Restaurante* pivo = array[(esq + dir) / 2];

     while (i <= j) {
          while (array[i]->avaliacao < pivo->avaliacao || ((array[i]->avaliacao == pivo->avaliacao) && strcmp(array[i]->nome, pivo->nome) < 0)) {
               (*comp)++;
               i++;
          }

          while (array[j]->avaliacao > pivo->avaliacao || ((array[j]->avaliacao == pivo->avaliacao) && strcmp(array[j]->nome, pivo->nome) > 0)) {
               (*comp)++;
               j--;
          }
...
void quicksort (Restaurante** array, int n, long* comp, long* mov) {
...
int main() {
     ...
     colecaoRestaurante ordenados; //array de saida
    ...
    quicksort (ordenados.restaurantes, ordenados.tamanho, &comparacoes, &movimentacoes);
    clock_t fim = clock();
